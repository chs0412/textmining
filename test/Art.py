...
def getlists(x,writer):
    ...
    idx=1
    realidx=(index*10 )+ idx
    while True:
        
        try:
            print(realidx)
            url = "https://www.thingiverse.com/search?page=" + str(realidx) + "&per_page=20&sort=popular&type=things&q=&category_id="+x
...
cateId=str(sys.argv[3])
print(category,index,cateId)
f = open(category+str(index)+".csv", "w")
writer=csv.writer(f)
column=["title","name","likes","tags","commentsCount","comments","makes","remixes","apps","date","url","id","img"]
writer.writerow(column)
            
getlists(cateId,writer)
